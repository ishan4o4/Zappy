...
import { replyWithDropdown } from "../../../utils/bankHelpers.js";
import User from "../../../models/User.js";

export default {
  async execute(message, args, user, bankTimeouts) {
    if (!user?.bank.hasAccount) return message.reply("❌ You don't have a bank account.");
    if (user.bank.loan <= 0 && (!user.bank.debt || user.bank.debt <= 0)) return message.reply("✅ You don't have any loans or debt to repay!");

    const amount = parseInt(args[1]);
    if (isNaN(amount) || amount <= 0) return message.reply("❌ Please enter a valid repayment amount.");

    let repayRemaining = amount;
    let repayFromReserved = 0, repayFromBankBalance = 0, repayFromWallet = 0, negativeWalletDebt = 0;
    let loanRepaid = 0, debtRepaid = 0;

    if (user.bank.loan > 0) {
      const loanPayment = Math.min(repayRemaining, user.bank.loan);

      if (user.bank.reserved > 0) {
        repayFromReserved = Math.min(loanPayment, user.bank.reserved);
        user.bank.reserved -= repayFromReserved;
        loanRepaid += repayFromReserved;
      }

      const remainingLoanPayment = loanPayment - repayFromReserved;
      if (remainingLoanPayment > 0 && user.bank.bankBalance > 0) {
        repayFromBankBalance = Math.min(remainingLoanPayment, user.bank.bankBalance);
        user.bank.bankBalance -= repayFromBankBalance;
        loanRepaid += repayFromBankBalance;
      }

      const stillRemainingLoan = loanPayment - repayFromReserved - repayFromBankBalance;
      if (stillRemainingLoan > 0) {
        repayFromWallet = Math.min(stillRemainingLoan, Math.max(0, user.balance));
        user.balance -= repayFromWallet;
        loanRepaid += repayFromWallet;

        const finalLoanRemainder = stillRemainingLoan - repayFromWallet;
        if (finalLoanRemainder > 0) {
          user.balance -= finalLoanRemainder;
          negativeWalletDebt += finalLoanRemainder;
          loanRepaid += finalLoanRemainder;
        }
      }

      user.bank.loan -= loanRepaid;
      if (user.bank.loan <= 0) {
        user.bank.loan = 0;
      ...
      repayRemaining -= loanPayment;
    }

    if (repayRemaining > 0 && user.bank.debt > 0) {
      debtRepaid = Math.min(repayRemaining, user.bank.debt);
      user.bank.debt -= debtRepaid;
      user.balance -= debtRepaid;
      if (user.balance < 0 && negativeWalletDebt === 0) {
        negativeWalletDebt = Math.abs(user.balance);
      }
    }

    const totalRepaid = loanRepaid + debtRepaid;

    await user.save();

    const embed = new EmbedBuilder()
      .setColor("Blue")
      .setTitle("💵 Loan/Debt Repayment")
      .setDescription(
        `✅ Repayment Processed!\n\n` +
        `💰 Total Repaid: ${totalRepaid}\n` +
        (loanRepaid > 0 ? `💳 Loan Repaid: ${loanRepaid}\n` : "") +
        (debtRepaid > 0 ? `🏦 Debt Repaid: ${debtRepaid}\n` : "") +
        `\nPayment Breakdown:\n` +
        `🔒 From Reserved Balance: ${repayFromReserved}\n` +
        `🏦 From Bank Balance: ${repayFromBankBalance}\n` +
        `🪙 From Wallet Balance: ${repayFromWallet}\n` +
        (negativeWalletDebt > 0 ? `⚠️ Negative Wallet Debt: ${negativeWalletDebt}\n` : "") +
        `\nCurrent Status:\n` +
        `💳 Remaining Loan: ${user.bank.loan}\n` +
        `🏦 Outstanding Debt: ${user.bank.debt || 0}\n` +
        `🪙 Wallet Balance: ${user.balance}${user.balance < 0 ? " (negative)" : ""}\n` +
        `🔒 Reserved Balance: ${user.bank.reserved}\n\n` +
        (user.bank.loan === 0 && (!user.bank.debt || user.bank.debt === 0) ? "🎉 All loans and debts are paid off!" : "💡 Keep repaying!")
      );
