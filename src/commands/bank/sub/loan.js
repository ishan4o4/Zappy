...
import { calculateDueDate, replyWithDropdown } from "../../../utils/bankHelpers.js";
...
export default {
  async execute(message, args, user, bankTimeouts) {
    ...
    if (!user?.bank.hasAccount) return message.reply(`❌ You need a bank account first! (${PREFIX}bank acc register)`);

    if (user.bank.loan > 0) return message.reply("❌ You already have a loan outstanding. Repay first.");

    const amount = parseInt(args[1]);
    if (isNaN(amount) || amount <= 0)
      return message.reply("❌ Please enter a valid loan amount.");

    const periodInput = (args[2] || "month").toLowerCase();
    const periodCount = Math.max(parseInt(args[3]) || 1, 1);

    const validPeriods = ["day", "days", "week", "weeks", "month", "months"];
    ...
    if (periodCount > maxAllowed)
      return message.reply(
        `❌ Loan period exceeds maximum allowed.\nMaximum for ${normalizedPeriod}: ${maxAllowed}`
      );

    if (amount > user.bank.reserved)
      return message.reply(`❌ Loan amount exceeds your reserved balance (${user.bank.reserved}).`);
    ...
    switch (normalizedPeriod) {
      case "day":
        periodInterestRate = baseMonthlyInterestRate / 30;
        actualDuration = `${periodCount} day${periodCount > 1 ? "s" : ""}`;
        break;
      case "week":
        periodInterestRate = (baseMonthlyInterestRate / 30) * 7;
        actualDuration = `${periodCount} week${periodCount > 1 ? "s" : ""}`;
        break;
      case "month":
        periodInterestRate = baseMonthlyInterestRate;
        actualDuration = `${periodCount} month${periodCount > 1 ? "s" : ""}`;
        break;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `✅ Loan Details:\n\n` +
          ...
          `📈 Interest Rate: ${(periodInterestRate * 100).toFixed(5)}% per ${normalizedPeriod}\n` +
          `💸 Total Interest: ${totalInterest}\n` +
          `🎯 Total to Repay: ${totalRepayment}\n` +
          `⏰ Due Date: ${dueDate.toLocaleString()}\n\n` +
          `🏦 New Bank Balance: ${user.bank.bankBalance}\n\n` +
          `🚨 Loans auto-collect on due date. Pay early to avoid debt.`
      );
