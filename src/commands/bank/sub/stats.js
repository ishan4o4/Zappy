  ...
  async execute(message, args, user, bankTimeouts) {
    ...
    const totals = await User.aggregate([
      {
        $group: {
          ...
          totalBankBalance: { $sum: "$bank.bankBalance" },
          totalReserved: { $sum: "$bank.reserved" },
          totalLoans: { $sum: "$bank.loan" },
          totalDebt: { $sum: "$bank.debt" }
        }
      }
    ]);

    const totalWalletBalance = totals[0]?.totalWallet || 0;
    const totalBankBalance = totals[0]?.totalBankBalance || 0;
    const totalReservedBalance = totals[0]?.totalReserved || 0;
    const totalLoans = totals[0]?.totalLoans || 0;
    const totalDebt = totals[0]?.totalDebt || 0;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: "Total Economy", value: `${TOTAL_ECONOMY.toLocaleString()}`, inline: true },
        { name: "Bank Reserve", value: `${bankReserve.toLocaleString()}`, inline: true },
        { name: "Reserve Usage", value: `${progressBar} \`${usedReserve}%\``, inline: false },

        { name: "Money Distribution", value: "\u200B", inline: false },
        { name: "In Wallets", value: `${totalWalletBalance.toLocaleString()}`, inline: true },
        { name: "In Bank Accounts", value: `${totalBankBalance.toLocaleString()}`, inline: true },
        { name: "In Reserved Accounts", value: `${totalReservedBalance.toLocaleString()}`, inline: true },
        ...
        { name: "Total Outstanding Loans", value: `${totalLoans.toLocaleString()}`, inline: true },
        { name: "Total Outstanding Debt", value: `${totalDebt.toLocaleString()}`, inline: true },
        { name: "Users in Debt", value: `${usersInDebt.toLocaleString()} users`, inline: true },
