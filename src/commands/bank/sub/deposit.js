...
export default {
  async execute(message, args, user, bankTimeouts) {
    ...
    if (amount > user.balance) return message.reply("❌ You don't have enough balance in your wallet to deposit.");
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `✅ Deposit Complete!\n\n` +
        `💰 Deposited: ${amount} to bank balance\n` +
        `🏦 New Bank Balance: ${user.bank.bankBalance}\n` +
        `🪙 Remaining Wallet: ${user.balance}\n\n` +
        `⚠️ Bank balance is taxed 2% monthly (no interest).\n` +
        `For growth, consider Reserved Balance instead!`
      );
