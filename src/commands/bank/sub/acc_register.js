...
import { generateUniqueBankId, createDropdown, setupBankTimeout, replyWithDropdown } from "../../../utils/bankHelpers.js";
import User from "../../../models/User.js";

export default {
  async execute(message, args, user, bankTimeouts) {
    if (user?.bank.hasAccount) return message.reply("⚠️ You already have a bank account!");
    if (!user) return message.reply("❌ You need to register first!");
    if (user.balance < 5000) return message.reply("❌ You need 5,000 reserved to open a bank account.");
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `✅ You successfully opened a bank account with **5,000 reserved** (earns 3% monthly interest).\n\n` +
        `🏷️ Bank Account ID: ${newBankAccountId}\n\n` +
        `Use Account Info to view balances,\nDeposit funds to Bank Balance for use.\n\n` +
        `⚠️ Loans auto-collect when due. Pay promptly!`
      );
