...
export default {
  async execute(message, args, user, bankTimeouts) {
    ...
    if (isNaN(amount) || amount <= 0) return message.reply("❌ Please enter a valid amount to increase your reserved balance.");
    if (amount > user.balance) return message.reply("❌ You don't have enough balance to reserve this amount.");

    let actualAmountToReserve = amount;
    let debtPaid = 0;

    if (user.bank.debt && user.bank.debt > 0) {
      debtPaid = Math.min(amount, user.bank.debt);
      user.bank.debt -= debtPaid;
      actualAmountToReserve -= debtPaid;
    }

    user.balance -= amount;
    user.bank.reserved += actualAmountToReserve;

    await user.save();

    const embed = new EmbedBuilder()
      .setColor("Blue")
      .setTitle("🔒 Reserved Balance Transaction")
      .setDescription(
        `✅ Transaction Processed!\n\n` +
        (debtPaid > 0 ? 
          `💳 Debt Payment: ${debtPaid} paid towards debt\n` +
          `💰 Reserved: ${actualAmountToReserve} added to reserved balance\n\n` +
          `⚠️ Remaining Debt: ${user.bank.debt}\n` :
          `💰 Added: ${amount} to reserved balance\n\n`) +
        `🔒 New Reserved Balance: ${user.bank.reserved}\n\n` +
        `📈 Benefits:\n` +
        `• Earns 3% monthly interest\n` +
        `• Lower loan interest rates\n` +
        `• Higher max loan limits\n\n` +
        `⚠️ Reserved balance cannot be withdrawn, but grows monthly!`
      );
