...
import { payDebtFromEarnings, replyWithDropdown } from "../../../utils/bankHelpers.js";
...
export default {
  async execute(message, args, user, bankTimeouts) {
    ...
    if (!user.bank.bankBalance || amount > user.bank.bankBalance) return message.reply("❌ Insufficient bank balance.");

    user.bank.bankBalance -= amount;

    const actualAmount = await payDebtFromEarnings(user, amount);
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `✅ Withdrawal Complete!\n\n` +
        `💸 Withdrawn: ${amount} from bank balance\n` +
        (actualAmount < amount 
          ? `💳 Debt Payment: ${amount - actualAmount} paid towards debt\n💰 To Wallet: ${actualAmount}\n⚠️ Remaining Debt: ${user.bank.debt}\n\n`
          : `💰 To Wallet: ${actualAmount}\n\n`) +
        `🏦 Remaining Bank Balance: ${user.bank.bankBalance}\n` +
        `🪙 New Wallet Balance: ${user.balance}\n\n` +
        `💡 Your money is now in your wallet and ready to spend!`
      );
